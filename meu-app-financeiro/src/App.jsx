...
// =====================================================
// COMPONENTES UI LOCAIS (ESTILO VIDRO CLARO SUAVE)
// =====================================================
const Card = ({ children, className }) => (
  <div className={`rounded-3xl border border-white/40 bg-white/30 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.05)] overflow-hidden ${className || ""}`}>
    {children}
  </div>
);
...
function UserProfile({ isOpen, onClose, onLogout, user, contas, cartoes, onDisconnectConta, onDisconnectCartao, onOpenPremium, onOpenPrivacy, onOpenDeleteAccount, onUpdateName }) {
  ...
  return (
    <div className="fixed inset-0 z-[240] bg-black/20 backdrop-blur-md transition-opacity">
      <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} className="absolute left-0 top-0 h-full w-full max-w-sm bg-white/30 backdrop-blur-xl border-r border-white/40 shadow-2xl overflow-y-auto flex flex-col">
        <div className="relative h-48 bg-gradient-to-br from-white/30 via-white/20 to-white/30 p-6 text-gray-800 overflow-hidden">
          ...
          <div className="absolute top-full -translate-y-[105%] left-8 z-20">
            <div className="relative group">
              <div className="size-24 rounded-[32px] bg-white/50 backdrop-blur-md p-1 shadow-xl rotate-3 group-hover:rotate-0 transition-transform duration-500 overflow-hidden border border-white/40">
                <img src={user.photo || DEFAULT_AVATAR} className="w-full h-full object-cover rounded-[28px]" alt="Avatar" />
              </div>
              <label className="absolute -bottom-1 -right-1 bg-white/60 backdrop-blur-md text-gray-800 p-2 rounded-2xl cursor-pointer shadow-lg hover:scale-110 transition-transform border border-white/40">
                <Sparkles size={12} />
        ...
        <div className="mt-14 px-6 space-y-6 pb-8">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {editingName ? (
                <div className="flex items-center gap-2 w-full">
                  <input value={tempName} onChange={(e) => setTempName(e.target.value)} className="flex-1 rounded-2xl border border-white/40 bg-white/60 backdrop-blur-md px-3 py-2 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-white/50" />
                  <button onClick={() => { onUpdateName(tempName.trim() || "Usuário"); setEditingName(false); }} className="rounded-2xl bg-white/60 backdrop-blur-md border border-white/40 px-3 py-2 text-xs font-black text-gray-800 shadow-sm">Salvar</button>
                  <button onClick={() => { setTempName(user.name); setEditingName(false); }} className="rounded-2xl border border-white/40 bg-white/40 backdrop-blur-md px-3 py-2 text-xs font-bold text-gray-800">Cancelar</button>
                </div>
              ) : (
          ...
          <button onClick={onOpenPremium} className="w-full group relative bg-white/30 backdrop-blur-md border border-white/40 p-5 rounded-[28px] shadow-sm hover:shadow-md transition-all overflow-hidden">
          ...
          <div className="grid gap-2">
            ...
            {contas.length ? contas.map((c) => (
              <div key={c.id} className="w-full rounded-2xl bg-white/40 backdrop-blur-md border border-white/40 p-4 shadow-sm flex items-center justify-between gap-3">
              ...
              <div className="rounded-2xl border border-dashed border-white/40 bg-white/30 backdrop-blur-md p-5 text-center text-xs text-gray-600">Espaço reservado para suas contas conectadas via Open Banking.</div>
            )}
            {cartoes.length ? cartoes.map((cartao) => (
              <div key={cartao.id} className="w-full rounded-2xl bg-white/40 backdrop-blur-md border border-white/40 p-4 shadow-sm flex items-center justify-between gap-3">
            ...
            <button onClick={onLogout} className="w-full p-4 bg-white/40 backdrop-blur-md text-rose-600 rounded-2xl flex items-center gap-3 font-bold text-sm shadow-sm border border-white/40 hover:bg-rose-100/50 transition-colors"><LogOut size={18} /> Sair da conta</button>
            <button onClick={onOpenDeleteAccount} className="w-full p-4 bg-white/30 backdrop-blur-md text-gray-600 rounded-2xl flex items-center gap-3 font-medium text-sm shadow-sm border border-white/40 hover:bg-rose-100/50 hover:text-rose-600 hover:border-rose-300/50 transition-all opacity-80 hover:opacity-100"><Trash2 size={18} /> Excluir minha conta</button>
          </div>
        </div>
      </motion.div>
    </div>
...
function MenuButton({ icon, label, onClick }) {
  return (
    <button onClick={onClick} className="w-full p-4 bg-white/40 backdrop-blur-md text-gray-800 rounded-2xl flex items-center justify-between font-bold text-sm shadow-sm border border-white/40 hover:bg-white/60 transition-all">
...
function PremiumModal({ isOpen, onClose, onContinue }) {
  ...
  return (
    <div className="fixed inset-0 z-[260] bg-black/20 backdrop-blur-md flex items-end md:items-center justify-center p-4">
      <div className="w-full max-w-md bg-white/50 backdrop-blur-xl border border-white/40 rounded-t-3xl md:rounded-3xl p-6 shadow-2xl">
        ...
        <div className="mt-5 rounded-2xl bg-white/40 border border-white/40 p-4 space-y-3">
...
function TermometroSobrevivencia({ gastoAtual, metaMensal }) {
  ...
  return (
    <div className="bg-white/30 backdrop-blur-md p-3 rounded-2xl shadow-sm border border-white/40 mt-3">
      ...
      <div className="relative h-5 w-full bg-white/40 rounded-lg overflow-hidden border border-white/40"><div className="h-full transition-all duration-500" style={{ width: `${Math.min(percentualGasto, 100)}%`, backgroundImage: getTermometroGradient() }} /></div>
...
function TelaInicialLogin({ onLogin }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-50 via-white to-gray-100 p-6 text-center">
      <div className="w-full max-w-md rounded-[32px] bg-white/30 backdrop-blur-xl border border-white/40 p-8 shadow-[0_25px_70px_rgba(0,0,0,0.1)]">
        <div className="mx-auto mb-6 flex items-center justify-center">
          ...
          <div
            style={{ display: "none" }}
            className="items-center justify-center rounded-full bg-white/60 backdrop-blur-md px-6 py-4 border border-white/40"
          >
        ...
        <div className="mt-8 grid gap-4 w-full">
          <Button onClick={onLogin} className="w-full bg-white/40 !text-gray-800 border border-white/40 hover:bg-white/60 font-bold rounded-2xl shadow-lg">
...
function ChatGemini() {
  ...
  function appendAttachments(fileList, kind) { 
    ...
    setAttachments((prev) => [...prev, ...novas]); 
  }
  ...
  async function toggleRecording() { 
    ...
    try { 
      ...
      recorder.onstop = () => { 
        ...
        const now = new Date(); 
        const name = `Áudio ${now.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}`; 
  ...
  function handleSend() { 
    ...
    setTimeout(() => { 
      setMensagens((prev) => [...prev, { 
        id: `bot-${Date.now()}`, 
        role: "bot", 
        text: "Entendido! Processando aqui... (Em breve vou integrar isso de verdade no seu banco de dados)." 
      }]); 
    }, 1000); 
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <div className="fixed top-4 left-4 z-[200]">
        <button onClick={() => setProfileOpen(true)} className="size-10 rounded-full border border-white/40 bg-white/50 backdrop-blur-md shadow-lg active:scale-90 transition-transform overflow-hidden">
      ...
      <div className="flex-shrink-0 pt-14 px-3 sm:px-6 max-w-6xl mx-auto w-full">
        <div className="bg-white/30 backdrop-blur-xl rounded-3xl border border-white/40 shadow-2xl p-3">
          ...
          <div className="rounded-2xl bg-white/40 backdrop-blur-md border border-white/40 shadow-lg overflow-hidden mb-3">
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-6 max-w-6xl mx-auto w-full pb-4 pt-2">
          ...
          <Card>
            <CardContent className="p-3">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                <div className="rounded-2xl bg-white/30 backdrop-blur-md p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Entradas do mês</div><div className="text-base font-semibold text-gray-800">{formatCurrency(totalEntradasMes)}</div></div>
                <div className="rounded-2xl bg-white/30 backdrop-blur-md p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Despesas do mês</div><div className="text-base font-semibold text-gray-800">{formatCurrency(totalDespesasMes)}</div></div>
                <div className="rounded-2xl bg-white/30 backdrop-blur-md p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Saldo do mês</div><div className="text-base font-semibold text-gray-800">{formatCurrency(saldoMes)}</div></div>
                <div className="rounded-2xl bg-white/30 backdrop-blur-md p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Meta de gastos</div><div className="text-base font-semibold text-gray-800">{formatCurrency(metaMensal)}</div><div className="text-[10px] mt-0.5 text-gray-500">Usado: {percentualMeta.toFixed(0)}%</div></div>
              </div>
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Tooltip contentStyle={{ backgroundColor: '#ffffffcc', border: '1px solid #e5e7eb', color: '#1f2937', backdropFilter: 'blur(8px)' }} />
          ...
          <Card>
            <CardContent className="p-3">
              ...
              <div className="flex flex-row gap-2 items-stretch">
                <div className="flex-[8] rounded-2xl bg-white/30 backdrop-blur-md border border-white/40 p-2 flex flex-col items-center justify-center overflow-visible">
                ...
                <div className="flex-[3] rounded-2xl bg-white/30 backdrop-blur-md border border-white/40 p-2 flex flex-col gap-1.5 min-w-[120px]">
                  ...
                  <div className="text-[8px] text-gray-500 text-center">Aviso de limite</div>
                </div>
              </div>
            ...
            <CardContent className="grid gap-2 p-3">
              ...
              {Object.entries(lancamentosFiltrados.reduce((acc, l) => { if (!acc[l.data]) acc[l.data] = []; acc[l.data].push(l); return acc; }, {})).sort((a, b) => (a[0] < b[0] ? 1 : -1)).map(([data, itens]) => (
                <div key={data} className="grid gap-1.5">
                  ...
                  {itens.map((l) => (
                    <div key={l.id} className="rounded-xl bg-white/30 backdrop-blur-md border border-white/40 p-2 flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-[10px] text-gray-600">{l.categoria} • {l.natureza}{l.conta_id ? ` • ${getContaNomeById(l.conta_id)}` : ""}</div>
                      </div>
                      <div className="flex items-center gap-2">
        ...
        {activeTab === "fixos" && (
          <Card>
            <CardContent className="grid gap-3 p-3">
              ...
              <div className="grid gap-2">
                {gastosFixos.map((g) => (
                  <div key={g.id} className="rounded-xl bg-white/30 backdrop-blur-md border border-white/40 p-2 flex justify-between items-center">
                    <div>
            ...
            <CardContent className="p-3 grid gap-2">
              <div className="grid md:grid-cols-3 gap-2 items-end">
                <div className="grid gap-0.5">
                  ...
                  <select value={tipoAnalise} onChange={(e) => setTipoAnalise(e.target.value)} className="rounded-xl bg-white/40 backdrop-blur-md border border-white/40 p-1.5 text-sm text-gray-800">
                ...
                <div className="grid gap-0.5">
                  ...
                  <select value={analiseSelecionada} onChange={(e) => { const valor = e.target.value; if (tipoAnalise === "receita") setAnaliseSelecionadaReceita(valor); else setAnaliseSelecionadaDespesa(valor); }} className="rounded-xl bg-white/40 backdrop-blur-md border border-white/40 p-1.5 text-sm text-gray-800">
                ...
                <div className="grid gap-0.5">
                  ...
                  <select value={periodoAnalise} onChange={(e) => setPeriodoAnalise(Number(e.target.value))} className="rounded-xl bg-white/40 backdrop-blur-md border border-white/40 p-1.5 text-sm text-gray-800">
                    ...
                    <option value={4} className="bg-white">Últimos 4 meses</option>
                    <option value={5} className="bg-white">Últimos 5 meses</option>
                    <option value={6} className="bg-white">Últimos 6 meses</option>
                  </select>
                </div>
              </div>
              <div className="text-xs text-gray-600">Evolução mensal da categoria selecionada</div>
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={serieAnaliseEspecial}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#d1d5db" />
                  <XAxis dataKey="mes" ticks={mesesReferenciaAnalise} tickFormatter={formatMonthLabel} interval={0} minTickGap={0} tickMargin={8} tick={{ fontSize: 10, fill: '#4b5563' }} />
        ...
        <div className="w-full text-center mt-3 pb-2">
          <div className="inline-block bg-white/30 backdrop-blur-md px-3 py-1 rounded-full border border-white/40 shadow-sm">
      ...
      {previewLancamento && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-md flex items-center justify-center z-50">
          <div className="bg-white/60 backdrop-blur-xl rounded-[32px] p-5 w-full max-w-md grid gap-3 shadow-2xl border border-white/40">
            ...
            <div className="flex items-center justify-between rounded-2xl bg-white/30 backdrop-blur-md px-3 py-3 border border-white/40"><span className="text-sm font-semibold text-gray-700">Valor do lançamento</span><Input inputMode="decimal" pattern="[0-9.,]*" value={formatCurrency(previewLancamento.valor)} onChange={(e) => setPreviewLancamento(prev => prev ? { ...prev, valor: parseCurrencyInput(e.target.value) } : prev)} className="w-24 border-none bg-transparent text-right text-lg font-bold focus:ring-0 text-gray-800" /></div>
            <div className="flex items-center justify-between rounded-2xl bg-white/30 backdrop-blur-md px-3 py-2 border border-white/40"><span className="text-sm font-semibold text-gray-700">Descrição</span><Input value={previewLancamento.descricao} onChange={(e) => setPreviewLancamento(prev => prev ? { ...prev, descricao: e.target.value } : prev)} className="w-40 border-none bg-transparent text-right font-bold focus:ring-0 text-gray-800" /></div>
            <div className="grid grid-cols-2 gap-2">
              <div className="grid gap-0.5"><span className="text-xs text-gray-500">Data</span><div className="relative group"><input type="date" value={previewLancamento.data} onChange={(e) => { const novaData = e.target.value; setDataSelecionada(novaData); setPreviewLancamento(prev => prev ? { ...prev, data: novaData } : prev); }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" /><div className="flex items-center justify-between rounded-xl bg-white/30 backdrop-blur-md px-2 py-1.5 border border-white/40 group-hover:border-gray-300 transition-all"><span className="text-sm font-semibold text-gray-800">{new Date(previewLancamento.data).toLocaleDateString("pt-BR")}</span><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-500 group-hover:text-gray-700 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div></div></div>
              <div className="grid gap-0.5"><span className="text-xs text-gray-500">Tipo</span><div className="flex bg-white/30 backdrop-blur-md rounded-xl p-0.5 border border-white/40"><button onClick={() => setPreviewLancamento(prev => prev ? { ...prev, tipo: "receita" } : prev)} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${previewLancamento.tipo === "receita" ? "bg-white/80 text-gray-800 shadow" : "text-gray-600"}`}>Receita</button><button onClick={() => setPreviewLancamento(prev => prev ? { ...prev, tipo: "despesa" } : prev)} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${previewLancamento.tipo === "despesa" ? "bg-white/80 text-gray-800 shadow" : "text-gray-600"}`}>Despesa</button></div></div>
            </div>
            <div className="grid gap-0.5"><span className="text-xs text-gray-500">Categoria</span><select value={previewLancamento.categoria} onChange={(e) => setPreviewLancamento(prev => prev ? { ...prev, categoria: e.target.value } : prev)} className="rounded-xl bg-white/30 backdrop-blur-md border border-white/40 p-2 text-sm text-gray-800">{(previewLancamento?.tipo === "receita" ? categoriasReceita : categoriasDespesa).map((cat) => (<option key={cat} value={cat} className="bg-white">{cat}</option>))}</select></div>
      ...
      {privacyOpen && (
        <div className="fixed inset-0 z-[270] bg-black/20 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white/60 backdrop-blur-xl border border-white/40 rounded-2xl p-5 shadow-2xl overflow-y-auto max-h-[80vh]">
      ...
      {deleteAccountModalOpen && (
        <div className="fixed inset-0 z-[280] bg-black/20 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white/60 backdrop-blur-xl border border-white/40 rounded-2xl p-5 shadow-2xl space-y-3">
