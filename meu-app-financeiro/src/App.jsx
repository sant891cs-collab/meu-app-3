...
const Button = ({ children, onClick, className, variant }) => {
  ...
  else if (variant === "receita") base += "bg-emerald-500 hover:bg-emerald-600 text-white backdrop-blur-sm "; // NOVO: botão verde para receita
...
export default function AppFinanceiroCompleto() {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-100 via-white to-gray-200 relative">
      ...
      {/* Área fixa superior - AGORA COM GRADIENTE MAIS ESCURO E BACKDROP-BLUR DESTACADO */}
      <div className="flex-shrink-0 pt-14 px-3 sm:px-6 max-w-6xl mx-auto w-full relative z-10">
        <div className="bg-gradient-to-b from-gray-200/80 to-gray-100/80 backdrop-blur-md rounded-3xl p-3 mb-2 border border-white/30 shadow-md">
          {aviso && (
            <div className="text-xs text-gray-600 text-center rounded-full bg-white/40 backdrop-blur-sm px-3 py-1 border border-white/30 mb-2">
              {aviso}
            </div>
          )}

          <Card className="mb-2 border-white/20">
            <CardContent className="grid gap-2 p-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <Button variant="receita" onClick={() => registrarMovimentoManual("receita")}>
                  Adicionar Receita
                </Button>
                <Button variant="destructive" onClick={() => registrarMovimentoManual("despesa")}>
                  Adicionar Despesa
                </Button>
              </div>
            </CardContent>
          ...
          <div className="flex overflow-x-auto whitespace-nowrap gap-1 rounded-2xl bg-white/40 backdrop-blur-md p-1 shadow-sm border border-white/30">
          ...
          <Card>
            <CardContent className="p-3">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                {/* Cards de resumo com cores diferentes e backdrop-blur destacado */}
                <div className="rounded-2xl bg-emerald-50/70 backdrop-blur-sm p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Entradas do mês</div><div className="text-base font-semibold text-gray-800">{formatCurrency(totalEntradasMes)}</div></div>
                <div className="rounded-2xl bg-rose-50/70 backdrop-blur-sm p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Despesas do mês</div><div className="text-base font-semibold text-gray-800">{formatCurrency(totalDespesasMes)}</div></div>
                <div className="rounded-2xl bg-blue-50/70 backdrop-blur-sm p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Saldo do mês</div><div className="text-base font-semibold text-gray-800">{formatCurrency(saldoMes)}</div></div>
                <div className="rounded-2xl bg-amber-50/70 backdrop-blur-sm p-2 border border-white/40 shadow-sm"><div className="text-xs text-gray-600">Meta de gastos</div><div className="text-base font-semibold text-gray-800">{formatCurrency(metaMensal)}</div><div className="text-[10px] mt-0.5 text-gray-500">Usado: {percentualMeta.toFixed(0)}%</div></div>
              </div>
